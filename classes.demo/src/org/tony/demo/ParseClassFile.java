...
public class ParseClassFile
{
  ...
  static Map<String, String> const_utf8_info = new HashMap<String, String>();
  static
  {
        ...
        switch (tag)
        {
          ...
          case IFC.CONSTANT_Utf8:
            ...
            if (len != -1)
            {
              ...
              const_utf8_info.put(String.valueOf(i), value);
              readCount += len;
              fmt("Index:%d,value:%s", i, value);
            }
            break;
      ...
      for (int i = 0; i < count; ++i)
      {
        ...
        if (len != -1)
        {
          ...
          for (int j = 0; j < attrCount; j++)
          {
            ...
            {
              readCount += len;
              int c_name_index = Util.byteArrayToInt(Arrays.copyOfRange(temp, 0, 2));
              int c_byte_len = Util.byteArrayToInt(Arrays.copyOfRange(temp, 2, 6));
              // fmt("\t 属性名称:%s,属性长度:%d", const_utf8_info.get(String.valueOf(c_name_index)), c_byte_len);
              if (c_byte_len > 0)
              {
                temp = new byte[c_byte_len];
                fis.read(temp);
              }
            }
          }
          fmt("\t字段：%s%s (%s)", sb.toString(), const_utf8_info.get(String.valueOf(nameIndex)),
              const_utf8_info.get(String.valueOf(descriptorIndex)));
        }
      }
    }
  }
    ...
    if (len != -1)
    {
      ...
      for (int i = 0; i < mCount; i++)
      {
        ...
        if (len != -1)
        {
          ...
          int iflag = Util.byteArrayToInt(Arrays.copyOfRange(temp, 0, 2));
          for (MethodAccFlag flag : MethodAccFlag.values())
          {
            if ((flag.value & iflag) == flag.value)
            {
              sb.append(flag.label + " ");
          ...
          for (int j = 0; j < attrCount; j++)
          {
            ...
            if (len != -1)
            {
              ...
              int c_name_index = Util.byteArrayToInt(Arrays.copyOfRange(temp, 0, 2));
              int c_byte_len = Util.byteArrayToInt(Arrays.copyOfRange(temp, 2, 6));
              System.out.println("c_byte_len:" + c_byte_len);
              int newI = ((temp[2] << 24) + (temp[3] << 16) + (temp[4] << 8) + (temp[5] << 0));
          ...
          fmt("\t 方法：%s%s();(%s)", sb.toString(), const_utf8_info.get(String.valueOf(nameIndex)),
              const_utf8_info.get(String.valueOf(descriptorIndex)));
        }
      }
    }
